...
function replaceIter(code) {
    ...
    let iters = [...code.matchAll(iter_regex)]

    // translated_code = ""
    code = code.replace(iter_regex, expandIter)

    // grab the substring (until ;) and expand it
    // for (iter in iters) {
    //     // result = expandIter(match[0]);
    //     // result = expandIter(iter[1], iter[2], iter[3])
    //     console.log(result)

    //     // code = code.substring(
    // }
    // console.log(iters);
    // console.log(iters[0].length);
