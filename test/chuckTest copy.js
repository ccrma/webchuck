...
const testSuite = [
  ...
  new Test(5, "Sync test set/get int and float, webchuck and js", async () => {
    ...
    let test1 =
      (await aChuck.getInt("GLOBAL_INT")) == 1 && (await aChuck.getFloat("GLOBAL_FLOAT")) == 1.0;
    outputBox.innerHTML += "Get OK " + test1 + "<br>";
    aChuck.setInt("GLOBAL_INT", 2);
    aChuck.setFloat("GLOBAL_FLOAT", 2.0);
    await new Promise((resolve) => setTimeout(resolve, 300));
    let test2 =
      (await aChuck.getInt("GLOBAL_INT")) == 2 && (await aChuck.getFloat("GLOBAL_FLOAT")) == 2.0;
  ...
  new Test(6, "Test shred management, add, remove, replace", async () => {
    ...
    let replaced =
      (
        await aChuck.replaceCode(
          `<<< "REPLACED Shred 1 with a new program\n", "" >>>; 1::second => now;`,
        )
      ).newShred == 1;
    await new Promise((resolve) => setTimeout(resolve, 100));
    ...
    await new Promise((resolve) => setTimeout(resolve, 100));
    return removed == true && replaced == true && active == true && removed2 == true;
  }),
